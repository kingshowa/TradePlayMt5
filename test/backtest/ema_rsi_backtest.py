...
def backtest_real_data_managed(
    ...
    print(f"\n🚀 EMA-RIS Backtest Started | {symbol}")
    ...
    # CSV output
    with open("trades_outcome_rsi.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
    ...
    for i in range(cut_off, len(series._candles) - 1):
        ...
        with open("trades_outcome_rsi.csv", "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                trade.candle.time,
                trade.direction,
    ...
    # =========================
    df = pd.read_csv("trades_outcome_rsi.csv")
