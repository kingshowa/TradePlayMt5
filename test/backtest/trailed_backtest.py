...
def backtest_real_data_managed(
    ...
    # CSV output
    with open("trades_outcome_trailed.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
    ...
    for i in range(cut_off, len(series._candles) - 1):
        ...
        with open("trades_outcome_trailed.csv", "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                trade.candle.time,
                trade.direction,
    ...
    print(f"✅ Trailed backtest complete. {trades_count} trades executed.")

    # =========================
    # Results & Equity Curve
    # =========================
    df = pd.read_csv("trades_outcome_trailed.csv")
    df["CumulativePnL"] = df["PnL"].cumsum()

    plt.figure(figsize=(12, 6))
    plt.plot(df["CumulativePnL"], label="Trailed Equity Curve")
    plt.title(f"Trailed Backtest Equity Curve - {symbol}")
