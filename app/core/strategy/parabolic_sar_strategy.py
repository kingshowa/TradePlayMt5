...
class ParabolicSarStrategy:
    ...
    Indicator roles:
    - PSAR: entry trigger and optional close signal
    - EMA: optional trend direction filter
    - ADX: optional trend strength filter
    - ATR: risk-management value passed to external risk engine

    The strategy only generates StrategySignal objects.
    SL/TP are always None because the risk engine owns them.
    We only suggest a SL on the first dot of PSAR trend
    """
    ...
    def __init__(
        ...
        close_signal_adx_limit: float = 20,
        max_candles: int = 300,
    ):
        ...
        self.close_signal_adx_limit = close_signal_adx_limit
    ...
    def update(self, candle: Candle) -> Optional[StrategySignal]:
        ...
        Order:
        1. Update all enabled indicators.
        2. If trade is open, check PSAR close signal.
        3. If flat, check PSAR flip entry signal.
        """
    ...
    def _close_adx_allows(self) -> bool:
        ...
        return self.current_adx >= self.close_signal_adx_limit

    # ---------------------------------------------------------
    # Signal builder
    # ---------------------------------------------------------
    def _build_entry_signal(self, direction: str, candle: Candle) -> StrategySignal:
        psar_sl = self._psar_stop_loss(direction)
    ...
    def _psar_stop_loss(self, direction: str) -> Optional[float]:
        """
        Use the first PSAR dot of the new trend as the proposed stop loss.

        BUY  -> PSAR dot below price
        SELL -> PSAR dot above price

        The risk manager can later compare this SL against ATR-based SL
        and choose the safer or more optimal level.
        """
